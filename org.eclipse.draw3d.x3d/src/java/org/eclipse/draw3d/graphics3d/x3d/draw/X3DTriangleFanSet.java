...
/**
 * A triangle fan set graphic primitive.
 * 
 * @author Kristian Duske
 * @version $Revision$
 * @since 09.06.2009
 */
public class X3DTriangleFanSet extends X3DTriangleSet {
	...
	public ListIterator<X3DNode> getNodeIterator() {
		...
		X3DNode triangleFanSet = new X3DNode("TriangleFanSet");

		if (m_vertices.size() < 3)
			throw new Graphics3DException(
					"Triangle fan set must have at least 3 coordinates");
		...
		triangleFanSet.addAttribute(new X3DAttribute("fanCount", m_vertices
				.size()));

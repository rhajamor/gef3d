...
public class Figure3DHelper {
    ...
    private void paintChildren2D(Graphics i_graphics) {
        ...
        if (!(m_figuresFriend.figure instanceof IHostFigure3D))
            return;

        IHostFigure3D hostFigure = (IHostFigure3D) m_figuresFriend.figure;

        Collection<IFigure> children = hostFigure.getChildren();
        if (children.isEmpty())
            return;
        ...
        if (repaint2D) {
            ...
            Graphics textureGraphics = g3d.activateGraphics2D(hostFigure,
                bounds.width, bounds.height, hostFigure.getAlpha(),
                hostFigure.getBackgroundColor());
             ...
             * textureManager.clearTexture(figure, figure.getBackgroundColor(),
             * figure.getAlpha()); Graphics textureGraphics =
             * textureManager.getGraphics(); Graphics textureGraphics =
        ...
        try {
            ...
            try {
                ...
                ConnectionLayer connectionLayer = hostFigure.getConnectionLayer(null);

                // paint the connections
                if (connectionLayer != null) {
                    IVector3f location3D = hostFigure.getPosition3D().getLocation3D();

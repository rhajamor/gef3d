...
public class Figure3D extends Figure implements IFigure3D {
    ...
    public IFigure findFigureAt(int i_sx, int i_sy, TreeSearch i_search) {
        ...
        if (i_search != null && i_search.prune(this))
            return null;

        IFigure hit = null;
        List<IFigure> children = getChildren();
        for (IFigure child : children) {
            if (!(child instanceof IFigure3D) && child.isVisible()) {
                if (i_search != null)
    ...
    public IFigure findMouseEventTargetAt(int i_sx, int i_sy) {
        ...
        for (IFigure child : children) {
            if (!(child instanceof IFigure3D) && child.isVisible()) {
                hit = child.findMouseEventTargetAt(i_sx, i_sy);
    ...
    public ISurface getSurface() {
        ...
        if (m_surface == null)
            m_surface = new FigureSurface(this);
    ...
    public void setPreferredSize3D(IVector3f i_preferredSize3D) {
        ...
        Dimension size = preferredSize3D.setVector3fAsDimension(i_preferredSize3D);
    ...
    public String toString() {
        ...
        strb.append(getLocation3D().getX()).append(",").append(
            getLocation3D().getY()).append(",").append(getLocation3D().getZ()).append(
            ")");
        strb.append(", size (");
        strb.append(getSize3D().getX()).append(",").append(getSize3D().getY()).append(
            ",").append(getSize3D().getZ()).append(")");

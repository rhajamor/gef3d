...
public class DispatchingConnectionLayerHelper {
    ...
    public boolean add(IFigure i_figure, Object i_constraint, int i_index) {
        ...
        if (i_constraint != CONSTRAINT_LAYER && i_figure instanceof Connection
            ...
            ConnectionLayer distributedLayer = getConnectionLayerOf3DHost(fig3DHost);
     ...
     *             if connection has different 3D hosts
     */
     ...
     * 
     * @param i_figure
     *            the figure for which we need the connection layer
     */
    private ConnectionLayer getConnectionLayerOf3DHost(IFigure3D i_figure) {

        if (i_figure == null)
            return null;

        ConnectionLayer layer = distributedLayersMap.get(i_figure);
        if (layer == null && (i_figure instanceof IHostFigure3D)) {
            IHostFigure3D hostFigure = (IHostFigure3D) i_figure;

            layer = hostFigure.getConnectionLayer(factory);
            if (layer != null) {
                host.add(layer, CONSTRAINT_LAYER);
                distributedLayersMap.put(i_figure, layer);
            } else {
